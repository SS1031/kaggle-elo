...
train_df = pd.read_csv('../data/inputs/train.csv')
test_df = pd.read_csv('../data/inputs/test.csv')
historical_trans_df = pd.read_csv('../data/inputs/historical_transactions.csv')
new_merchant_trans_df = pd.read_csv('../data/inputs/new_merchant_transactions.csv')
...
historical_trans_group_df['hist_purchase_date_diff'] = (
        historical_trans_group_df['hist_purchase_date_max'] - historical_trans_group_df['hist_purchase_date_min']
).dt.days
historical_trans_group_df['hist_purchase_date_average'] = historical_trans_group_df['hist_purchase_date_diff'] / \
                                                          historical_trans_group_df['hist_card_id_size']
...
new_merchant_trans_group_df['new_hist_purchase_date_diff'] = (
        new_merchant_trans_group_df['new_hist_purchase_date_max'] - new_merchant_trans_group_df[
    'new_hist_purchase_date_min']).dt.days
new_merchant_trans_group_df['new_hist_purchase_date_average'] = new_merchant_trans_group_df[
                                                                    'new_hist_purchase_date_diff'] / \
                                                                new_merchant_trans_group_df['new_hist_card_id_size']
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(train_df, train_df['outliers'].values)):
    ...
    print(strLog)
    trn_data = lgb.Dataset(train_df.iloc[trn_idx][train_columns],
                           label=target.iloc[trn_idx])  # , categorical_feature=categorical_feats)
    val_data = lgb.Dataset(train_df.iloc[val_idx][train_columns],
...
plt.tight_layout()
plt.savefig('../data/outputs/imp/lgbm_importances.png')
##submission
logger.info("Prepare submission")
sub_df = pd.DataFrame({"card_id": test_df["card_id"].values})
sub_df["target"] = predictions
sub_df.to_csv("../data/outputs/elo_world_high_score_without_belnding.csv", index=False)
